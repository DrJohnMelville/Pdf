...
public class UncoloredTilingPatternMatrix : UncoloredTilingPattern
{
    ...
    protected override async ValueTask DoPaintingAsync(ContentStreamWriter csw)
    {
        ...
        await csw.SetNonstrokingColorExtendedAsync(NameDirectory.Get("P1"), 0.77, 0.2, 0.0);
        
        DrawCircle(csw);
        
        csw.SaveGraphicsState();
        await csw.SetNonstrokingColorExtendedAsync(NameDirectory.Get("P1"), 0.2, 0.8, 0.4);
        csw.ModifyTransformMatrix(Matrix3x2.CreateTranslation((float)(224.96 - 99.92), 0));
        DrawCircle(csw);
        csw.RestoreGraphicsState();

        csw.SaveGraphicsState();
        await csw.SetNonstrokingColorExtendedAsync(NameDirectory.Get("P1"), 0.3, 0.7, 1.0);
        csw.ModifyTransformMatrix(Matrix3x2.CreateTranslation((float)(224.96 - 99.92)/2f, 100f));
        DrawCircle(csw);
        csw.RestoreGraphicsState();

        await csw.SetNonstrokingColorExtendedAsync(NameDirectory.Get("P1"), 0.5, 0.2, 1.0);
        csw.SaveGraphicsState();
