	...
	internal class ColorSpace
	{
		...
		protected internal void  getBoxes()
		{
			...
			while (true)
			{
                ...
                len = (long)Melville.CSJ2K.Icc.ICCProfile.getInt(boxHeader, 0);
				if (len == 1)
                    len = Melville.CSJ2K.Icc.ICCProfile.getLong(boxHeader, 8); // Extended
				// length
                type = Melville.CSJ2K.Icc.ICCProfile.getInt(boxHeader, 4);
				
				// Verify the contents of the file so far.
				if (i == 0 && type != Melville.CSJ2K.j2k.fileformat.FileFormatBoxes.JP2_SIGNATURE_BOX)
				{
					throw new ColorSpaceException("first box in image not " + "signature");
				}
				else if (i == 1 && type != Melville.CSJ2K.j2k.fileformat.FileFormatBoxes.FILE_TYPE_BOX)
				{
					throw new ColorSpaceException("second box in image not file");
				}
				else if (type == Melville.CSJ2K.j2k.fileformat.FileFormatBoxes.CONTIGUOUS_CODESTREAM_BOX)
				{
					throw new ColorSpaceException("header box not found in image");
				}
				else if (type == Melville.CSJ2K.j2k.fileformat.FileFormatBoxes.JP2_HEADER_BOX)
				{
					break;
				}
			...
			for (boxStart += 8; boxStart < headerBoxEnd; boxStart = (int) (boxStart + len))
			{
                ...
                type = (int)Melville.CSJ2K.Icc.ICCProfile.getInt(boxHeader, 4);
				
				switch (type)
				{
					
					case Melville.CSJ2K.j2k.fileformat.FileFormatBoxes.IMAGE_HEADER_BOX: 
						ihbox = new ImageHeaderBox(in_Renamed, boxStart);
						break;
					
					case Melville.CSJ2K.j2k.fileformat.FileFormatBoxes.COLOUR_SPECIFICATION_BOX: 
						csbox = new ColorSpecificationBox(in_Renamed, boxStart);
						break;
					
					case Melville.CSJ2K.j2k.fileformat.FileFormatBoxes.CHANNEL_DEFINITION_BOX: 
						cdbox = new ChannelDefinitionBox(in_Renamed, boxStart);
						break;
					
					case Melville.CSJ2K.j2k.fileformat.FileFormatBoxes.COMPONENT_MAPPING_BOX: 
						cmbox = new ComponentMappingBox(in_Renamed, boxStart);
						break;
					
					case Melville.CSJ2K.j2k.fileformat.FileFormatBoxes.PALETTE_BOX: 
