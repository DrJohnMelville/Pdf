	...
	public class ImgReaderPGX:ImgReader, EndianType
	{
		...
		public ImgReaderPGX(System.IO.Stream in_Renamed)
		{
			...
			try
			{
				...
				if (tmp.Equals("LM"))
					byteOrder = Melville.CSJ2K.j2k.io.EndianType_Fields.LITTLE_ENDIAN;
				else if (tmp.Equals("ML"))
					byteOrder = Melville.CSJ2K.j2k.io.EndianType_Fields.BIG_ENDIAN;
				else
					throw new System.IO.IOException("Not a PGX file");
						...
						if (isSigned)
						{
							for (i = blk.uly; i < mi; i++)
							{
								...
								switch (byteOrder)
								{
									...
									case Melville.CSJ2K.j2k.io.EndianType_Fields.LITTLE_ENDIAN: 
									...
									case Melville.CSJ2K.j2k.io.EndianType_Fields.BIG_ENDIAN: 
										for (k = (i - blk.uly) * blk.w + blk.w - 1, j = (blk.w << 1) - 1; j >= 0; k--)
										{
