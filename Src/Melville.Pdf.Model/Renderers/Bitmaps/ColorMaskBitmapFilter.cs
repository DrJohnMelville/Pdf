...
internal readonly struct ColorMaskBitmapFilter: IBitmapFilter
    {
        ...
        public ColorMaskBitmapFilter(int[] values, int bitsPerComponent, IColorSpace colorSpace)
        {
        ...
        public unsafe byte AlphaForByte(byte* target, int row, int col) =>
            blueRange.IsInRange(target[0]) &&
            greenRange.IsInRange(target[1]) &&
            redRange.IsInRange(target[2])
                ? (byte)0
                : (byte)255;
    }
