...
public class RecordedGlyph : ITrueTypePointTarget
{
    ...
    {
        points.Add(new(point.X, point.Y, isContourStart, isContourEnd, onCurve));
    }

    private record struct GlyphPoint(
        double X,
        double Y,
        bool Begin,
        bool End,
    ...
    private record struct BoundingBox(
        double x1,
        double y1,
        double x2,
        double y2,
        ...
        Matrix3x2 Matrix)
    {
        ...
        private Vector2 TransformedPoint(double x, double y) =>
            Vector2.Transform(new Vector2((float)x, (float)y), Matrix);
    }
    ...
    private static void DrawUnitRect(ScaledDrawContext dc, Pen? unitPen) =>
