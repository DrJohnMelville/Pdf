...
internal readonly partial struct CmapFormat12Parser
{
    ...
    [SFntField(nameof(numGroups))] private readonly SequentialMapGroup[] mapGroups;
    public static async ValueTask<ICmapImplementation> ParseAsync(PipeReader input)
    {
        ...
        return new CmapFormat12Implementation(rec.mapGroups);
    }
}
...
internal readonly partial struct SequentialMapGroup
{
    ...
    public uint MapGlyph(uint character) => 
        character < startCharCode || character > endCharCode? 0:
        character - startCharCode + startGlyphId;
}

internal class CmapFormat12Implementation (SequentialMapGroup[] maps) : ICmapImplementation
{
    public IEnumerable<(int Bytes, uint Character, uint Glyph)> AllMappings()
    {
        foreach (var map in maps)
        {
            ...
            {
                yield return new (4, i, map.MapGlyph(i));
            }
        }
    }
    ...
    public bool TryMap(int bytes, uint character, out uint glyph)
    {
        ...
        while (true)
        {
            switch (range.Length)
            {
                ...
                case 1:
                    return ReturnGlyph(out glyph, range[0].MapGlyph(character));
                case 2:
