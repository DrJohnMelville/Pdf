...
public class CcittCodeReaderTest
{
    ...
    {
        var reader = CreateSequence(input);
    ...
    private SequenceReader<byte> CreateSequence(string bits)
    {
    ...
    private byte BitsToBytes(bool[] arg) =>
        arg.Aggregate<bool, byte>(0, (prior, isone) => (byte)((prior << 1) | (isone ? 1 : 0)));

    private IEnumerable<bool> PadToByte(int length)
    {
        var extra = length % 8;
