...
public class ObjectStreamIndirectObject : OwnerContainedIndirectObject
{
    private readonly long containingStream;

    public ObjectStreamIndirectObject(int objectNumber, int generationNumber, ParsingFileOwner? owner, long containingStream) : base(objectNumber, generationNumber, owner)
    {
    ...
    protected override async ValueTask ReadFinalValue(ParsingFileOwner localOwner)
    {
        var referredObject = await owner.IndirectResolver
            .FindIndirect((int)containingStream, 0).DirectValueAsync().CA();
