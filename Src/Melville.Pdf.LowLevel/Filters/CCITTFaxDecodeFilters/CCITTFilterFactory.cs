...
{
    public static async ValueTask<IStreamFilterDefinition> EncoderAsync(PdfObject? arg) => 
        CcittCodecFactory.SelectEncoder(await ParseCcittOptionsAsync(arg).CA());

    public static async ValueTask<IStreamFilterDefinition> DecoderAsync(PdfObject? arg) => 
    ...
    public static ValueTask<CcittParameters> ParseCcittOptionsAsync(PdfObject? parameters) =>
        FromDictionaryAsync(parameters as PdfDictionary ?? PdfDictionary.Empty);
