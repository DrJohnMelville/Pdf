...
namespace Melville.Pdf.LowLevel.Writers.Builder
{
    public static class TreeCreator
    {
        public static PdfDictionary CreateTree<T>(
            ...
            where T : PdfObject, IPdfTreeKey<T> =>
    ...
    public readonly struct TreeCreator<T> where T : PdfObject, IPdfTreeKey<T>
    {
        ...
        private PdfDictionary CreateLeafNode(IList<(T Key, PdfObject Item)> ordered)
        {
            ...
            return new PdfDictionary(
                (finalArrayName, new PdfArray(InterleveKeysAndValues(ordered))),
                (KnownNames.Limits, new PdfArray(ordered.First().Key, ordered.Last().Key))
            );
        }
        ...
        private PdfDictionary TrivialTree(IReadOnlyList<PdfDictionary> leaves) =>
            new((finalArrayName, leaves[0].RawItems[finalArrayName]));

        private PdfDictionary TreeFromChunks(IReadOnlyList<PdfDictionary> leaves)
        {
            return new PdfDictionary(
                (KnownNames.Kids, new PdfArray(
                    ReduceLeaves(leaves).Select(CreateIndirectReference).ToList()))
            );
        }

        private IReadOnlyList<PdfDictionary> ReduceLeaves(IReadOnlyList<PdfDictionary> leaves) =>
            leaves.Count <= nodeSize ? 
                leaves :
                ReduceLeaves(leaves.Chunks(nodeSize).Select(MiddleNode).ToList());
        ...
        private PdfDictionary MiddleNode(IList<PdfDictionary> nodes)
        {
            ...
            return
                    new PdfDictionary(
                        (KnownNames.Kids, new PdfArray(nodes.Select(CreateIndirectReference).ToList())),
                        (KnownNames.Limits, new PdfArray(FirstKey(nodes.First()), LastKey(nodes.Last())))
                    );
        }
