...
namespace Melville.Pdf.LowLevel.Writers.ObjectWriters
{
    public static class StreamWriter
    {
        ...
        public static async ValueTask<FlushResult> Write(PipeWriter target, PdfObjectWriter innerWriter, PdfStream item,
            IObjectEncryptor encryptor)
        {
            await DictionaryWriter.Write(target, innerWriter, item.RawItems);
            target.WriteBytes(streamToken);
            await using var rawStream = encryptor.WrapReadingStreamWithEncryption(
                await item.GetEncodedStreamAsync());
